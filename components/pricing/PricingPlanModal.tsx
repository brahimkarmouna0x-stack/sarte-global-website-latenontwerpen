...
function priceCaption(price: PricingPrice): string {
  if (price.amount === null) return "Offerte Laten Ontwerpen";
  return "excl. btw";
}
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col gap-2 pr-10">
              ...
              >
                Je gaat verder via{" "}
                <em className="font-normal italic text-(--color-accent)">
                  WhatsApp
                </em>
              </h2>
              <p className="text-sm leading-relaxed text-(--color-text-secondary)">
                Controleer hieronder je gekozen pakket. Daarna openen we
                WhatsApp met je bericht al volledig ingevuld.
              </p>
            </div>
            ...
            <div className="mt-4 flex flex-col gap-2">
              ...
              <ul className="space-y-2 rounded-xl border border-(--color-border) bg-(--color-surface-2) px-4 py-3">
                {selection.tier.deliverables.map((item) => (
                  <li
                    key={item}
                    className="flex items-start gap-2 text-sm leading-snug text-(--color-text)"
                  >
            ...
            <p className="mt-3.5 text-center text-[11.5px] leading-snug text-(--color-text-muted)">
              Veilige doorverwijzing naar WhatsApp. We delen je gegevens nooit
              met derden.
            </p>
          </motion.div>
        </motion.div>
